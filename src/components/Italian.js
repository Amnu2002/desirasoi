...
export default function Italian(props) {
    const [articles, setarticles] = useState([]);
    const [loading, setloading] = useState(true)
    const [food, setfood] = useState('food')
    const searchName = useRef(null)
    const Apifetch = async () => {
            const apiurl = `https://api.edamam.com/search?q=${food}&app_id=f5421d68&app_key=d38aec9d400ec6fa6552a55690d8de81&cuisineType=italian`;
    ...
    useEffect(() => {
        ...
        const handleClick = async(e)=>{
            e.preventDefault()
            const apiurl = `https://api.edamam.com/search?q=${searchName.current.value}&app_id=f5421d68&app_key=d38aec9d400ec6fa6552a55690d8de81&cuisineType=italian`;
            setloading(true)
            const data = await fetch(apiurl);
            const parsedData = await data.json()
            setarticles(parsedData.hits);
